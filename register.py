...
class Registry(sp.Contract):
    def __init__(self, _admin):
        self.init(
                   admin = _admin, 
                   registry = sp.map(l={}, tkey = sp.TAddress, tvalue = sp.TInt))
    ...
    def addRecordAdmin(self, params):
        sp.verify(sp.sender == self.data.admin, "Invalid admin")
        self.data.registry[params.account] = params.amount

@sp.add_test(name = "Testing Registry Contract")
def testingRegistry():
    scenario = sp.test_scenario()
    sc = Registry(sp.address('tz1WxLmxr5a4BqFGeh6YyLhZhkgFpFihts1T'))
    scenario += sc
    ...
    scenario += sc.addRecordAdmin(account = sp.address('tz1-address-4'),
 amount = 5656).run(sender = sp.address('tz1WxLmxr5a4BqFGeh6YyLhZhkgFpFihts1T'))